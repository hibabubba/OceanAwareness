...
class LocationActivity : AppCompatActivity() {
 ...
 private suspend fun fetchJson() {
    ...
    fun create_activity(){//Første gang programmet kjører vises denne været for nå
        ...
        precipitation.text = "Nedbør: \n" + data?.product?.time?.get(3)?.location?.precipitation?.value + data?.product?.time?.get(3)?.location?.precipitation?.unit
    ...
    fun set_dataImage_forcast(liste: List<ImageButton>, list: List<TextView>) {
        ...
        for (nr in 0..47) {
            if (nr in 0..3) { //de 3 første har 2 mellom seg
                ...
                //sette iconene
                set_image(clouds!!, liste.get(nr), kl, precipitation!!)
                //hente og legge til info i liste_med_info
                liste_med_info.add( get_info(teller,tellerregn))



                //Legg til text klokkeslett
                for(textview in list){
                    list.get(nr).text = kl + ":00"
                }
    ...
    fun set_image(clouds: Double, icon: ImageButton, kl: String, precipitation: Double){
        ...
        if(kl.toInt() in 6..22  ){
            ...
            if(clouds in 0.0..12.5){
                ...
                checkRain(icon, precipitation, R.drawable.lettskyet)
                //icon.setImageResource(R.drawable.lettskyet)
            } else if(clouds in 37.5..62.5){
                checkRain(icon, precipitation, R.drawable.skyet)
                //icon.setImageResource(R.drawable.skyet)
            } else{ //63.5 og utover
                checkRain(icon, precipitation, R.drawable.overskyet)
    ...
    fun checkRain(icon: ImageButton, precipitation: Double, sky: Int){
        if(precipitation > 2.0){
            icon.setImageResource(R.drawable.nedbor)
        } else{
            icon.setImageResource(sky)
        }
    }
